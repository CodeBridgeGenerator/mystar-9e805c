...
import appraisalFakerFactory from "./appraisalFakerFactory";
...
const AppraisalFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = appraisalFakerFactory(fakerCount,userIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("appraisal").create(elem));
...
export default connect(mapState, mapDispatch)(AppraisalFakerDialogComponent);
